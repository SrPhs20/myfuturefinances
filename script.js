...
function atualizarTela() {
  ...
  filtrados.forEach((item) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="danger" onclick="remover(${indexOriginal})">Excluir</button>
      </td>
    `;
  ...
  contasOrdenadas.forEach((conta) => {
    ...
    div.innerHTML = `
      ...
      <div class="bill-actions">
        <button onclick="pagarContaFixa(${conta.indexOriginal})">Marcar como paga</button>
        <button class="danger" onclick="removerContaFixa(${conta.indexOriginal})">Excluir</button>
      </div>
    `;
...
function exportarDados() {
  const dados = JSON.stringify({ lancamentos, contasFixas }, null, 2);
